...
${m.html ?? ""}`.toLowerCase().includes(matching)) ?? null : messages[messages.length - 1] ?? null;
      ...
      if (readOnlyBlock) {
        verdict = "uncertain";
        reason = `blocked host: ${readOnlyBlock} is not on this run's allowed host list \u2014 pass --allow-host ${readOnlyBlock} (or add it to allowedHosts in spike.config.json) to let the run act there`;
        break;
      }
      if (ssoPopupBlock) {
        verdict = "uncertain";
...
function whoseFaultLabel(whose) {
