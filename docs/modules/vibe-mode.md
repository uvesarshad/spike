...
Keys and settings live in `chrome.storage.local` (`qaKeys`, `qaSettings`), not the Vault or `SettingsStore` — those are Node-only. What lite mode CANNOT do (needs the daemon): CLI planners/navigators (`claude`/`codex`/`gemini` CLI, or Ollama — nothing that spawns a process), the auto-fix loop (edits files on disk), and replay-clip download (`extension/panel.js` shows `'replay clips need Spike Core (lite mode is test-only).'` when the daemon isn't connected). `buildLiteConfig()`'s per-provider `liteUsable` flag is `false` for `nano` and `ollama` in the GENERIC provider list (nano needs the daemon to plan, ollama needs a local server) — this is separate from `LITE_NAVIGATOR_PROVIDERS`, which still lists `nano` as a valid NAVIGATOR pick via the dedicated rung-0 `nanoDeps` path in `runLite()`.
