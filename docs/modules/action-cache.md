...
`FileActionCache(rootDir)` writes records under `rootDir/v1/<hash-prefix>/<hash>.json`. The root defaults to `./.qa-action-cache` and is controlled by cfg.actionCacheDir / QA_ACTION_CACHE_DIR. The cache is off by default; enable it with cfg.actionCache, QA_ACTION_CACHE, or `qa run --action-cache`.
