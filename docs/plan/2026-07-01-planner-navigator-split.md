...
- Daemon parity: `src/vibe/service.ts` `vibe.config.get/set` + `src/config.ts` (`QA_NAVIGATOR_*` env) + `SettingsStore` handle the `navigator` field; `buildLiteConfig` in lite-engine adds it too.
