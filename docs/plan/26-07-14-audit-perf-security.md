...
Failure scenario: after `--install-service` + reboot, the ONLOGON-triggered daemon runs without the system-CA override, so any CLI-planner child it spawns (`gemini`, `claude`, …) hits the documented OAuth/TLS exit-41 failure again, silently, until the user manually relaunches `qa daemon` from a shell that has the var set.
