...
test/v1-v31 (*.ts) - Numbered integration spikes and focused regressions. These document capabilities such as clip recording, CDP logpoints, Nano availability probing, assertion policy, runtime data, action cache, telemetry redaction, gateway helpers, and video assertion script handling. They are reference code, not imported by the main test suites.
...
- When action-cache, assertion-policy, video assertion, runtime data, telemetry, or gateway coverage changes.
