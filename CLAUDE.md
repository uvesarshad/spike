...
- **Naming (post-rename):** npm package `spike-agent`, CLI binary `spike`, extension "Spike — QA testing agent", daemon "Spike Core" (OS service ids `com.spike-agent.daemon` / Windows task `Spike Core` / `spike-core.service`). Env prefix is `SPIKE_*`; `src/config.ts` reads `spike.config.json`. Two deprecation shims cover the old names and BOTH go at 1.0: `src/env-compat.ts` aliases any lingering `QA_*` var onto `SPIKE_*` (new name wins; one-time stderr notice, silence with `SPIKE_SUPPRESS_LEGACY_ENV_WARNING=1`), and `CONFIG_FILENAMES` falls back to `qa.config.json`. The npm-facing MCP *tool* name stays `qa_run` — it is semantic for the calling LLM, not a brand surface.
