...
- **Navigator/brain pins are set separately from the SettingsStore.** The saved `settings.json` here still has `planner: gemini:cli` (dead) and no `navigator`, so `loadConfig` resolves brain→`gemini:cli` (fails) and navigator→Nano. For a working run override per-role via env: `QA_PLANNER_PROVIDER=claude QA_PLANNER_MODE=cli` (brain) and `QA_NAVIGATOR_PROVIDER=… QA_NAVIGATOR_MODE=…` (navigator). Env beats the SettingsStore; a lone `QA_*_MODEL` partial-merges onto provider/mode.
...
- **External sites are read-only by the Tier-4 guard:** only `allowedHosts` (default `localhost`,`127.0.0.1`) may be clicked/typed; everywhere else navigation + looking is allowed but mutation is blocked and the run ends `uncertain` if a goal *needs* interaction. Note redirects: `mapleandsand.com` → `www.mapleandsand.com`, so allow BOTH via `QA_ALLOWED_HOSTS`. Only widen this for sites you're authorized to drive, and scope the task to non-destructive checks (no checkout/register/form submits).
