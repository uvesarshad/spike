...
| `spike run … --fix [--fix-on-uncertain] [--rebuild-timeout <s>] [--wait-for-url <url>]` | On a failed test, hand the fix prompt to your coding agent, wait for your change to show up (a short pause on localhost; otherwise until the page actually changes, up to 180 s, or until `--wait-for-url` answers), then re-test. Only a real failure is fixed — an "uncertain" result is left alone unless you add `--fix-on-uncertain`. If the change could not be confirmed the result says it may be for the old version. The side panel's Auto-fix button does the same and shows "Fixed and re-checked: passed" or "Still failing after N tries" |
| `spike tests list` / `quarantine <name> [--reason]` / `release <name>` | List saved tests with their last result; park a flaky one (it still runs and reports but cannot fail the suite) and un-park it. Also shows baseline status (`spike bless --list` still works) |
| `spike tests review` / `accept <name>` / `reject <name>` | Review changes a self-heal proposed but held back; apply one (rewrites the saved test and its Playwright copy) or discard it |
| `--budget <$>` on `run --spec`, `check`, `suite`, `replay --all`, `ci`, `schedule add`, `watch` (and `budgetUsd` on the `qa_run` / `site_check` / `tests_run` tools) | Spending limit for a whole batch. When it is reached the remaining flows/pages/tests are skipped, the result is "not sure" and says "stopped: spending limit reached ($X of $Y)"; each run is also limited to the money left. Unattended runs are capped by default: `schedule` and `watch` at **$1 per job per 24 hours** (`unattendedBudgetUsd` / `SPIKE_UNATTENDED_BUDGET_USD`), `ci` at **$2 per run** (`ciBudgetUsd` / `SPIKE_CI_BUDGET_USD`); an explicit `--budget` wins, and interactive runs are never capped unless you ask |
| `--stop-on-fail`, `--steps-per-flow <n>` on `run --spec`, `check`, `suite` | Stop at the first flow/page/test that fails; limit how many steps one flow may take (`suite`: same as `--max-steps`) |
...
| `spike dashboard [--port n] [--host addr] [--no-open]` | Open **Spike home** — past runs, saved tests (with changes waiting for review), your site map and last check, scheduled tests with spend vs limit, and setup (assistants found, which AI does what, which keys exist — never their values). It is also where you act: copy a fix prompt, re-run, save a run as a test, run all tests, accept or reject a proposed repair, park a flaky test, run or remove a schedule, or type a site and a sentence into **Test something**. Every action needs a per-session token the page embeds, so other sites in your browser cannot trigger them. Running `spike` with no arguments opens this page when Spike Core is running (otherwise it prints help and a tip), and each `spike run` result ends with a `details:` link to its page when it is up. Spike Core (`spike daemon`) serves the same page on `http://127.0.0.1:9420`. Listens on this machine only; requests with an unexpected `Host` header get 403 (DNS-rebinding guard); `--host` with any other address exposes past reports to your network and prints a warning |
...
A JSON Schema for this result ships in the package as `schema/slim-result.schema.json` (`node_modules/spike-agent/schema/`), for anyone building an agent or CI step on top of it.
...
- **A saved session** — `spike run … --storage-state <file>` reuses cookies and local storage captured from an earlier signed-in session. The side panel offers the same thing as **"Remember my login for tests"** (Spike Core only): a test that signs in and passes has its session saved to `~/.spike/state/<host>.json` (owner-readable only), and every later test on that site starts from it.

  **How this differs from just being logged in.** The command-line transports drive a persistent Chrome profile of their own, so once anything signs in there, *every* later run on that machine is still signed in — the browser simply remembers, the way your own browser does, and nothing is written by Spike. A saved session is the opposite trade: an explicit snapshot of cookies and local storage taken at one moment, kept in a file, and injected into an otherwise fresh session. That makes it portable (hand the file to CI, or to `--auth-fixture` so one login seeds a whole suite) and repeatable (every run starts from the same known state), but it is a *copy* — it goes stale when the session expires, it misses anything the app keeps outside cookies and local storage (IndexedDB, a service worker's cache), and the file is a live session: anyone who can read it is signed in as you, which is why it is written owner-only and why it should be a test account. `--via playwright` always starts from a fresh context, so there a saved session is the only way to arrive signed in.
