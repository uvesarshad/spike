...
export function toPlaywrightSpec(script: QaScript): string {
  ...
  for (const s of script.steps) {
    switch (s.type) {
      ...
      case 'extract':
        ...
        break;
      case 'assert_visual':
        lines.push(`  // ${s.mode === 'video' ? 'video' : 'visual'} check (judged by the QA subagent when replayed via \`spike replay\`):`);
  ...
  return `// Generated by spike-agent from passing run ${script.sourceRunId}
// Task: ${script.task.replace(/\n/g, ' ')}
// Replay without Playwright (and with $0 visual checks): spike replay ${script.name}
