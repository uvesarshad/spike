 ...
 * via SPIKE_TELEMETRY_EXPORTER=otlp. See docs/modules/telemetry.md. */
 ...
 * SPIKE_TELEMETRY_EXPORTER mid-run should call `resetDefaultTracer()` first. */
...
function buildTracerFromEnv(): TelemetryTracer {
  ...
  if (!endpoint) {
    console.error(
      'telemetry: SPIKE_TELEMETRY_EXPORTER=otlp is set but SPIKE_OTLP_ENDPOINT is missing — ' +
